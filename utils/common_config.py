...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20', 'drew-fish']:
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    elif p['train_db_name'] == 'drew-fish':
        from data.drew_fish import DrewFish
        dataset = DrewFish(transform=transform)
    ...
    elif p['train_db_name'] == 'custom':
        from data.drew_fish import DrewFish
        dataset = DrewFish(main_dir=MyPath.db_root_dir('custom'), transform=transform)
